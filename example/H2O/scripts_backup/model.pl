...
%model = (
  ...
  'basis',         "cc-pVQZ",         # make sure there are corresponding files in $dir_basis (see below)
  ...
  'nreference',   14,                 # number of orbitals used for searching reference orbitals
  ...
  'frozen_orbs',   [1,0,0,0], # C2v, which orbitals for each symmetry to use as frozen,
  'active_orbs',   [3,1,3,1], # C2v, which orbitals for each symmetry to use as active,
  'virtual_orbs',  [3,1,1,1], # which orbitals for each symmetry to use as virtual,
  ...
  # 'ncasscf_states', {'doublet', [1,1,1,0],
  'ncasscf_states', {'singlet', [1,0,0,0],
                     'triplet', [0,0,0,0]},
  ...
  'ntarget_states', {'singlet', [1,0,0,0],  # number of target states to calculate in each irreducible representation (IR)
                     'triplet', [0,0,0,0]},
  ...
  'scattering_states', {'doublet', [1,1,1,1], # 1/0 to run/not to run scattering calculation for a given spin-symmetry
                        'quartet', [0,0,0,0]},

  # Continuum basis set
  'use_GTO',             1,              # do you want to use a Gaussion type basis?
  'radius_GTO',         13,              # radius for which Gaussian basis was optimized (basis sets available for radius 10, 13, 15, 18) 'maxl_GTO',            5,              # the highest partial wave used in the continuum Gaussian basis
  'maxl_GTO',            5,              # the highest partial wave used in the continuum Gaussian basis
 ...
 # Energy grid
  'nescat',   "36          , 36          ,  36          , 500",              # number of input scattering energies in each subrange (input for R_SOLVE via namelist &rslvin)
